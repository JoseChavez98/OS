...
The architecture that \cite provides, solves the above problems by leveraging some unique properties of NFAs and unique features of the AP. For in-
stance, we utilize the connected components (disconnected subgraphs) in an NFA to merge enumeration paths and thereby take
...
prior work [25, 29], we observe enumeration paths converge at run time and implement dynamic convergence checks in AP. To solve the
...
\begin{itemize}
    ...
    \item Quick look to evaluation of the technique against important benchmarks.
